# Ansible Automation Platform Solution Guides

Solution guides are enterprise-grade technical references that show how to solve real operational problems with Ansible Automation Platform. Each guide includes architecture diagrams, executable YAML examples, validation steps, and a maturity path from proof-of-concept to production.

Browse all solution guides, filter by partner or type, and find contributing guidelines at the **[Solution Guides hub](https://ansible-tmm.github.io/solution-guides/)**.

---

**Ansible Automation Platform Features**

- [Automation Dashboard and Analytics - Solution Guide](https://access.redhat.com/articles/7136383)
- [Get started with EDA (Ansible Rulebook) - Solution Guide](https://access.redhat.com/articles/7136720)

**AI + Ansible Automation Platform**

- [AI Infrastructure automation with Ansible - Solution Guide](https://ansible-tmm.github.io/solution-guides/README-IA)
- [AIOps automation with Ansible - Solution Guide](https://ansible-tmm.github.io/solution-guides/README-AIOps)
- [Configuring Ansible Lightspeed intelligent assistant with Red Hat AI Inference Server on RHEL - Solution Guide](https://ansible-tmm.github.io/solution-guides/README-Intelligent-Assistant-RHAIIS)

**AIOps Integrations**

- [AIOps with Splunk and Event-Driven Ansible - Solution Guide](https://ansible-tmm.github.io/solution-guides/README-AIOps-Splunk-ITSI)
- [Automated Incident Remediation with IBM Instana - Solution Guide](https://ansible-tmm.github.io/solution-guides/README-Instana-AIOps)
- [Unlock AIOps with ServiceNow LEAP and Ansible MCP server - Solution Guide](https://ansible-tmm.github.io/solution-guides/README-AIOps-ServiceNow)

**Infrastructure Automation**

- [High-Availability AAP with EDB PostgreSQL Active-Passive DR - Solution Guide](https://ansible-tmm.github.io/solution-guides/README-EDB)

**Network Automation with Ansible Automation Platform**

- [Network Fact Gathering & Reporting - Solution Guide](https://access.redhat.com/articles/7123361)
- [Network Back Up and Configuration - Solution Guide](https://access.redhat.com/articles/7123366)

**ServiceNow ITSM Automation with Ansible Automation Platform**

- [ServiceNow ITSM Ticket Enrichment Automation - Solution Guide](https://ansible-tmm.github.io/solution-guides/README-ServiceNow-ITSM)
